<x-modal form_action="{{ route('admin.settings.update', $setting) }}" id="{{ $id }}">
    <x-slot:title>{{ __('general.einstellung_bearbeiten') }}: {{ $setting->getKey() }}</x-slot:title>
    <x-slot:body>
        <x-forms.input :default="$setting->getValue()" name="value" required>{{ __('general.wert') }}</x-forms.input>
    </x-slot:body>
    <x-slot:footer>
